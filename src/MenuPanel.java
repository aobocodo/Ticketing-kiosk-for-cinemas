...
public class MenuPanel extends JPanel {
	...
	public MenuPanel(CardLayout card, Container container){
		...
		lblNewLabel.setIcon(new ImageIcon(controlelement.getElement("photo",0)));//xml第一个电影的图片位置
		...
		JLabel Label = new JLabel(controlelement.getElement("title",0));//xml里第一个电影的名字
		Label.setHorizontalAlignment(SwingConstants.CENTER);
		Label.setFont(new Font("Times New Roman", Font.BOLD, 21));
		panel_2.add(Label);
		
		
		JLabel lblMin = new JLabel(controlelement.getElement("timelength",0));//xml里第一个电影的时间
		lblMin.setHorizontalAlignment(SwingConstants.CENTER);
		lblMin.setFont(new Font("Times New Roman", Font.BOLD, 21));
		panel_2.add(lblMin);
			
		
		JButton[] btnNewButton = new JButton[controlelement.getNumber()];//电影的数量
	
		for(int i=0; i<controlelement.getNumber(); i++){
			
			final int j=i;
			btnNewButton[i] = new JButton(controlelement.getElement("title", i));//xml所有电影的名字
